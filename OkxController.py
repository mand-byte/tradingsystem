...
class OkxController(Controller):
    ...
    async def every_min_task(self):
        ...
        if not isinstance(swap_account, str):
            DataStore.swap_account[self.exdata.id] = swap_account[0]
...
################################################################################################################################################################
    ...
    async def query_swap_subPosId(self,info:OrderInfoDB):
        ...
        if isinstance(result,str):
            logger.info(f'okx controller query_subPosId 错误 info={info.to_json()} err={result}')
            return
        for i in result:
        ...
        if result.status==Const.ORDER_STATUS_LIVE:
            ...
            if self.sdk.swap_copytrader:
                ...
                if isinstance(result,str):
                    ...
                    logger.info(f'okx close_swap_by_order 平单成功 {info.to_json()}')
                    if len(info.sl_id)>0:
